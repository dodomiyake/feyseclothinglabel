  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs tracking-[0.3em] text-gold-700 uppercase">Invoice</p>
          <h1 className="mt-1 font-serif text-3xl text-ink-950">{invoice.invoice_number}</h1>
          <p className="mt-1 text-sm text-neutral-600">For enquiry {invoice.enquiry.enquiry_number}</p>
        </div>
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-y-2 border-t border-ink-900/8 pt-4 text-sm">
            <div><dt className="text-xs text-neutral-600">Issue date</dt><dd className="text-ink-900">{formatDate(invoice.issue_date)}</dd></div>
            <div><dt className="text-xs text-neutral-600">Due date</dt><dd className="text-ink-900">{formatDate(invoice.due_date)}</dd></div>
          </div>
          <dl className="space-y-1.5 border-t border-ink-900/8 pt-4 text-sm">
            <div className="flex justify-between"><dt className="text-neutral-600">Subtotal</dt><dd>{formatCurrency(invoice.subtotal, invoice.currency)}</dd></div>
            <div className="flex justify-between"><dt className="text-neutral-600">Delivery fee</dt><dd>{formatCurrency(invoice.delivery_fee, invoice.currency)}</dd></div>
            {invoice.discount > 0 && <div className="flex justify-between"><dt className="text-neutral-600">Discount</dt><dd>-{formatCurrency(invoice.discount, invoice.currency)}</dd></div>}
      ...
      {latestPayment && (
        <Card>
          ...
          <CardBody className="space-y-2 text-sm">
            <div className="flex justify-between"><dt className="text-neutral-600">Amount submitted</dt><dd>{formatCurrency(latestPayment.amount_paid, invoice.currency)}</dd></div>
            <div className="flex justify-between"><dt className="text-neutral-600">Submitted on</dt><dd>{formatDate(latestPayment.payment_date)}</dd></div>
            <div className="flex justify-between"><dt className="text-neutral-600">Status</dt><dd className="capitalize">{latestPayment.status.replace("_", " ")}</dd></div>
